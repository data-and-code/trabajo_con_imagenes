...
![RE](img/tarea_snake_3_1_2_gradient.png)
...
Seguidamente guardamos en X, un remodelado de las imagenes limpias 2 (que son las imagenes donde limpiamos el musculo pectoral) en forma de matriz de: $322 \times (256 \cdot 256) \rightarrow 322 \times 65536$ donde cada fila es una imagen y cada columna es un pixel (esto lo podemos ver como si "aplanaramos" las imagenes en una sola fila).

Aunado a X,tenemos Y, donde guardamos la clasificación de las imagenes y de donde solo rescataremos la letra que identifica el tipo de tejido de cada imagen.
...
Ahora separaremos en dos grupos los conjuntos de datos de modo que tengamos un conjunto del 80% de los datos en forma de entrenamiento para los modelos que usaremos para el proposito del trabajo (clasificación de los tejidos) y un 20% de prueba para medir su eficacía en la predicción.

```{python}
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.20, random_state=100, stratify=y)
```

Ya que tenemos separados los datos, crearemos un objeto PCA en el cual ajustaremos todos nuestros datos origiales (X) y mediremos su varianza explicada para buscar disminuir el numero de componentes de cada imagen (consiguiendo disminuir la complejidad de los datos, el costo computacional, tiempo y más recursos).

```{python}
pca = PCA(n_components=256, whiten=True).fit(X)
```

Graficamos la varianza explicada acumulada y buscamos el componente en el que la varianza total explicada sea mayor al 75%, en ese momento terminamos de buscar.
...
Podemos observar en la gráfica que apartir del componente 12 preservamos el 75% de la información.

Ahora con esto creamos nuevamente un objeto PCA estableciendolo en 12 componentes y ajustandolo a nuestros datos. Una vez nuestro modelo se entrenó con nuestros datos, transformamos los datos que habíamos separados anteriormente.
...
Ahora, buscaremos hacer una SVC la cual nos clasifique el tipo de tejido de la imagen.

![RE](img/tarea_3_2_3.png)

Pero ya que nuestros conjuntos de datos no son lineales.

![RE](img/tarea_3_0_clean.png)

Usaremos un kernel de funcion de base radial.
...
Compararemos con un modelo de Bosque Aleatorio (Random Forest) probando diferentes numeros de estimadores. 
...
Podemos observar que tuvo un mejor desempeño al poder identificar mejor la clase F y la clase G. además, mostró una mejoría en su precisión comparado con el modelo SVC.
